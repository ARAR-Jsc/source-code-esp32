...
void wifi_init_sta()
{
    ...
    ////    },
    ////};
	////memcpy((char*)wifi_config2.sta.ssid, ssid, 32);
	////memcpy((char*)wifi_config2.sta.password, password, 64);
	printf("value of sta.ssid: %s\n", wifi_config2.sta.ssid);
	printf("value of sta.ssid: %s\n", wifi_config2.sta.password);
    ...
    if (bits & WIFI_CONNECTED_BIT) {
        ESP_LOGI(TAG, "connected to ap SSID:%s password:%s",
                 wifi_config2.sta.ssid, wifi_config2.sta.password);
			ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_STA, &wifi_config2) );
...
static void udp_server_task(void *pvParameters)
{
    ...
    while (1) {
        ...
        while (1) {
			...
			memcpy((char*)wifi_config2.sta.ssid, SSIDX, 32);
			memcpy((char*)wifi_config2.sta.password, PWDX, 64);
			////ESP_ERROR_CHECK(esp_wifi_stop());
			////ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_STA) );
			ESP_ERROR_CHECK(esp_wifi_set_config(WIFI_IF_STA, &wifi_config2) );
    			////wearLevellingStorage(wifiCredential, rx_buffer);
    			wearLevellingStorage(wifiCredential, "test wifi credentials");
...
////
////    uart_write_bytes(ECHO_UART_PORT_NUM, "xin chao\n", 9);
////    ////while (1) {
...
uart_write_bytes(uart_num, "xin chao", 7);
printf("end of checking uart \n\n");
